...
const App = () => {
    ...
    if (!selectedItem || quantity <= 0) {
      setMessage("Invalid item or quantity");
      return;
    }
    setIsProcessing(true);
    try {
      const response = await fetch(`http://localhost:5000/stock/${action}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ itemName: selectedItem, quantity: parseInt(quantity) })
      });
      if (response.ok) {
        setMessage(`Stock successfully ${action === "add" ? "added" : "removed"}`);
        setSelectedItem("");
        setQuantity("");
        fetchStock();
      } else {
...
const handlePrintReport = () => {
    ...
    return (
        ...
        />
        <button onClick={() => handleStockUpdate("add")} disabled={isProcessing}>Add Stock</button>
        <button onClick={() => handleStockUpdate("remove")} disabled={isProcessing}>Remove Stock</button>
      </div>
      {message && <p>{message}</p>}
      <ul>
        {stockItems.map((item, index) => (
